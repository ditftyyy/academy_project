                                        ...
                                        @if (auth()->user()->hasRole('admin'))
                                            <div style="display: flex; column-gap: 10px;">
                                                @if (strtolower($status) == 'libur')
                                                    <form action="{{ url('/akademik/jadwal-kelas/' . $kelas->_id) }}" method="POST">
                                                        @csrf
                                                        <input type="hidden" name="hari" value="{{ $hari }}">
                                                        <button type="submit" name="status" value="masuk" class="btn btn-success btn-sm text-sm rounded">Masuk</button>
                                                    </form>
                                                @else
                                                    ...
                                                    <form action="{{ url('/akademik/jadwal-kelas/' . $kelas->_id) }}" method="POST">
                                                        @csrf
                                                        <input type="hidden" name="hari" value="{{ $hari }}">
                                                        <button type="submit" name="status" value="libur" class="btn btn-danger btn-sm text-sm rounded">Liburkan</button>
                                                    </form>
                                                @endif
                                            </div>
                                        @endif
                                                ...
                                                @if (auth()->user()->hasRole('admin'))
                                                    <div style="display: flex; justify-content: end; column-gap: 10px; margin: 10px 0px">
                                                        <button type="button" data-bs-toggle="modal" data-bs-target="#update-modal"
                                                            ...
                                                            onclick="setUpdateData('{{ $kelas->_id }}', '{{ $hari }}', {{ $index }}, '{{ $mp['jam_mulai'] }}', '{{ $mp['jam_selesai'] }}', '{{ $mp['mapel_id'] ?? '' }}', '{{ $mp['guru_id'] ?? '' }}', '{{ $mp['ruang_id'] ?? '' }}', '{{ $mp['mapel'] }}', '{{ $mp['guru'] }}', '{{ $mp['ruang'] }}')">
                                                            <i class="fa fa-edit"></i>
                                                        </button>
                                                        <form action="/akademik/jadwal-hapus/{{ $kelas->_id }}/{{ $hari }}/{{ $index }}" method="POST" style="display:inline">
                                                            @csrf @method('DELETE')
                                                            <button type="submit" class="btn btn-danger btn-sm text-sm rounded" onclick="return confirm('Hapus jadwal ini?')"><i class="fa fa-trash"></i></button>
                                                        </form>
                                                    </div>
                                                @endif
                                            @endforeach
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="{{ route('jadwal.update') }}" method="post">
                    ...
                    <div class="mb-3">
                        <label>Mapel</label>
                        <select class="form-select" name="mapel_id" id="edit-mapel-id" required>
                            <option value="">-- Pilih --</option>
                            @foreach ($mapels as $mapel)
                                <option value="{{ $mapel->_id }}">{{ $mapel->nama_mapel }}</option>
                            @endforeach
                        </select>
                        <input type="hidden" name="mapel" id="edit-mapel-nama">
                    </div>
                    <div class="mb-3">
                        <label>Guru</label>
                        <select class="form-select" name="guru_id" id="edit-guru-id" required>
                            <option value="">-- Pilih --</option>
                            @foreach ($gurus as $guru)
                                <option value="{{ $guru->_id }}">{{ $guru->guru_data['nama'] ?? $guru->nama_lengkap }}</option>
                            @endforeach
                        </select>
                        <input type="hidden" name="guru" id="edit-guru-nama">
                    </div>
                    <div class="row">
                    ...
                    <div class="mb-3">
                        <label>Ruang</label>
                        <select class="form-select" name="ruang_id" id="edit-ruang-id" required>
                            <option value="">-- Pilih --</option>
                            @foreach ($ruangs as $ruang)
                                <option value="{{ $ruang->_id }}">{{ $ruang->nama_ruang }}</option>
                            @endforeach
                        </select>
                        <input type="hidden" name="ruang" id="edit-ruang-nama">
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Tutup</button>
                        <button type="submit" class="btn btn-primary">Simpan</button>
...
<script>
    ...
    function setUpdateData(kelasId, hari, index, jamMulai, jamSelesai, mapelId, guruId, ruangId, mapelNama, guruNama, ruangNama) {
        ...
        document.getElementById('edit-guru-id').value = guruId;
        document.getElementById('edit-guru-nama').value = guruNama;
        document.getElementById('edit-ruang-id').value = ruangId;
        document.getElementById('edit-ruang-nama').value = ruangNama;
        // Karena select option value menggunakan id, kita set value id-nya saja
    }
</script>
@endsection