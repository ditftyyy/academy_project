                ...
                <button type="button" class="btn btn-primary dropdown-toggle" data-bs-toggle="dropdown"
                    ...
                    <strong>
                        ...
                        <span class="d-sm-inline d-none">
                            {{-- ============================================ --}}
                            {{-- MONGODB: Ambil nama dari field yang benar --}}
                            {{-- ============================================ --}}
                            @if (auth()->user()->hasRole('admin'))
                                {{ auth()->user()->profile['nama_lengkap'] ?? auth()->user()->username }}
                            @elseif (auth()->user()->hasRole('guru'))
                                {{ auth()->user()->guru_data['nama'] ?? auth()->user()->profile['nama_lengkap'] ?? auth()->user()->username }}
                            @elseif (auth()->user()->hasRole('siswa'))
                                {{ auth()->user()->siswa_data['nama'] ?? auth()->user()->profile['nama_lengkap'] ?? auth()->user()->username }}
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <form action="{{ route('set_role') }}" class="row g-3 px-4" method="post" enctype="multipart/form-data">
                            @csrf
                                ...
                                @foreach (explode(',', auth()->user()->role) as $value_role)
                                    @if ($value_role != 'root')
                                        <div style="display: flex; column-gap: 5px;">
                                            ...
                                            <label style="margin: 0" for="navbar-role-{{ $value_role }}">
                                                {{ ucfirst($value_role) }}
                                            </label>
                                        </div>
                                    @endif
